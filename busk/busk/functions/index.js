...
//post comment sends product information to cart database
exports.postcomment = functions.https.onRequest((request, response) => {
    cors(request, response, async () => {
        try {//nesecary information for request body
...
exports.moveToCompletedOrders = functions.https.onRequest(async (request, response) => {
    cors(request, response, async () => {
        try {
            ...
            const orderId = request.body.data.orderId; //extracts order id from my database
            ...
            //get the data from that order and set it as a doc in completed orders
